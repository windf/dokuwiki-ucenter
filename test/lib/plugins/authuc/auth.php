...
class auth_plugin_authuc extends DokuWiki_Auth_Plugin {
    protected $cnf;
     ...
     * @author  Matthias Grimm <matthiasgrimm@users.sourceforge.net>
     */
    public function getUserData($user, $isuid = 0){
        $user_info = false;
        $isuid = intval($isuid);
        if($data = $this->_uc_get_user($user, $isuid)){
            ...
            $user_info = array(
                'name' => $username,
                'mail' => $email,
                'grps' => $this->_get_user_group($username, 1),
                'uid' => $uid
            );
        }
        return $user_info;
...
//        }
    ...
    public function trustExternal($user, $pass, $sticky = false){
    	...
    	$checked = false;
    	if(!empty($user)){
    		list($uid, $username, $password, $email, $nickName) = $this->_uc_user_login($user, $pass);
    ...
    private function _uc_user_login($username, $password){
    	...
    	return array($return[0], $this->_convert_charset($return[1], 0), $return[2], $return[3], $return[5]);
    }
    ...
    private function _uc_get_user_full($username, $isuid = 0){
    	...
    	if($status) {
    		...
    		return array(
    				...
    				'grps' => $this->_get_user_group($status['username'], 1),
